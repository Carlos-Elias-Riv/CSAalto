...
def astar(h, starting_state, goaltest):
    ...
    Q = PriorityQueue()
    # g(inicial state) = 0 por lo tanto f(inicial state) = h(inicial state)
    Q.put((h(starting_state) + g[starting_state] ,starting_state))
    ...
    while not Q.empty() and not found:
        ...
        prio, state = Q.get()
        visited[state] = 1
        hijos = state.successors()
        #print(state)
        if goaltest(state):
            found = True
        if not found:
            for action, stateh in hijos:
                # f(suc)
                # if condicion: then abro ese camino
                
                if stateh not in visited or g[state] + action.cost < g[stateh]:

                    visited[stateh] = 1
                    predecessor[stateh] = (state, action) #(state, action)
                    # el costo de llegar a s, desde el initial state debería ser, la distancia que tenía mi papá + 1
                    g[stateh] = g[state] + action.cost
                    
                    Q.put((g[stateh] + h(stateh), stateh))
    
    # tenemos que irnos de reversa
    while state in predecessor.keys():
        list_actions.append(predecessor[state][1])
