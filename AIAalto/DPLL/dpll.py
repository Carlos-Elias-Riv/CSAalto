...
#     ## primero voy a construir todos los literals que tengo para poder encontrar los que ya trabajé
#     allliterals = set()
#     Q = []
#     # for clause in clauses:
#     #     for literal in clause:
#     #         allliterals = allliterals.add(literal[0])
...
#             l = None
#             falsecount = 0
#             # encuentro un literal no asignado
#             for literal in clause:
#                 if literal[0] not in valuation:
#                     l = literal
#                     break
#                 if literal[0] in valuation and valuation[literal[0]] == False:
#                     falsecount += 1
            
#             # significa que encontré un literal no asignado
#             if l is not None:
#                 tempclause = clause.copy()
#                 tempclause.remove(l)
#                 total = len(tempclause)
#                 # pruebo todos los demás
#                 if total > 0:
#                     for literal in tempclause:
#                         if literal[0] in valuation and valuation[literal[0]] == False:
#                             total -= 1
                    

#             # si hay un literal que no tiene valor y el total es 1, entonces es un literal unitario
#             if l is not None and total == 0:
#                 Q.append(l)
            
#         # significa que no encontre nada para inferir
#         if len(Q) == 0:
...
def unit_propagation(valuation, clauses):
    ...
    while True:
        for clause in clauses:
            unassigned_literals = [literal for literal in clause if literal[0] not in valuation]
            if len(unassigned_literals) == 1:
                l = unassigned_literals[0]
                false_literals = [literal for literal in clause if literal[0] in valuation and valuation[literal[0]] != literal[1]]
                if len(false_literals) == len(clause) - 1 :
                    Q.append(l)

            if len(unassigned_literals) == 0: # all are in the valuation
                false_literals = [literal for literal in clause if valuation[literal[0]] != literal[1]]
...
def dpll(valuation, clauses, variables):
    ...
    (satisfiability1, valuation1) = dpll(valuation, clauses, variables)

    if satisfiability1:
        return (True, valuation1)
    else:
        valuation[branching_variable] = False
        (satisfiability2, valuation2) = dpll(valuation, clauses, variables)
