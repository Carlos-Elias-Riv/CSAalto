...
def value_of(mdp, s, a, v, gamma):
    ...
    sum = 0
    for state in mdp.successor_states(s, a): 
...
def value_iteration(mdp, gamma, epsilon = 0.001):
   ...
   while not flag: 
      for state in mdp.states():
         tempdict = {action: value_of(mdp, state, action, v, gamma) for action in mdp.applicable_actions(state)}
         v_next[state] =tempdict[argmax(tempdict)]
      flag = all(abs(v[state] - v_next[state]) < (epsilon*(1-gamma))/(2*gamma) for state in mdp.states())
      v = v_next.copy()
...
def make_policy(mdp, optimal_values, gamma):
    ...
    ----------
    mdp : mdp.MDP
       Markov decision process object derived from class mdp.MDP.
