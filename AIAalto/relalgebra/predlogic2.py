...
class NOT:
    ...
    def fma2RA(self,dbSchema):
        ...
        #exit(1)
        freevars = self.subformula.freeVars()
        # Add U(x) as a conjunct for every variable
        fixedSubformula = reduce(AND,[ ATOM("U",[Var(x)]) for x in freevars])
        # Difference of the universal relation and the table obtained from the subformula
        return RAdifference(fixedSubformula.fma2RA(dbSchema), self.subformula.fma2RA(dbSchema))
