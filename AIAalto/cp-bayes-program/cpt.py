...
def exhaustive_enum(G, X, E, e):
   ...
   for assignment in assignments:
      ...
      for node in G.nodes:
         parents = {value : combined_dictionary[value] for value in node.parents if value in combined_dictionary}
         tmp *= evaluate_node(G, node.data_id, combined_dictionary[node.data_id], parents)
         # =========================================================
...
def sample(G, X, E, e):
   ...
   for variable in X:
      ...
      if variable in E:
         ...
         p = G.get_node_probability(variable, parents)
